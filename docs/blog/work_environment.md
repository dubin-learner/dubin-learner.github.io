...
### 基础配置
这部分配置是Vim本身自带的，配置完直接生效，具体含义都比较直白，不再赘述。
```vim
...
## Vim8.2安装Python3.6.8的支持
主要是为了使用clang-format进行代码格式化。

原本使用`clang-format`可执行文件也可以，但有个限制，选中一部分代码进行格式化时，这种方式不会考虑上下文中已有的格式。

例如，格式化某个函数中的某几行，本身函数体有缩进，这几行需要在这个基础上进一步缩进。

如果使用可执行文件，这几行就会默认从行首开始。因此需要使用clang-format.py。

首先，需要对Vim8.2进行重新编译，因为要增加编译选项：
```bash
...
--with-python3-command=~/tools/installs/bin/python3 \
--with-python3-config-dir=~/tools/installs/lib/python3.6/config-3.6m-x86_64-linux-gnu \
--prefix=~/tools/installs
```

其中，`--with-python-config-dir`需要指定的路径：
- 在python2中通常在安装路径的`lib/python2/config`；
- 在python3中可以直接执行安装路径下`bin/python3-config --configdir`获取。

注意看`./configure`执行后的log，是否有python相关的配置没有成功。如果没有问题，继续执行`make && make install`即可。

对于Python3.6.8来说，要支持Vim8.2在编译的时候需要额外开启选项`--enable-shared`；否则Vim只会找到静态库，然后提示库不可用：
```bash
E370: Could not load library libpython3.6m.a: /usr/lib/libpython3.6m.a: invalid ELF header
```

对于Python3.6.8的编译：`./configure --prefix=~/tools/installs --enable-shared`。

编译好之后，还需要在`.vimrc`中增加：`set pythonthreedll=~/tools/installs/lib/libpython3.6m.so.1.0`，这样Vim8.2中Python3.6.8的支持就完成了。

验证Vim中python支持是否成功的方法：
- 打开Vim，输入`:version`，查看python/python3前面是否已经是`+python`和`+python3`；
- 输入`:python print("hello")`和`:python3 print("hello")`查看是否可以正常输出"hello"。

至于对clang-format的脚本支持，还是到Vim配置中再说吧。
