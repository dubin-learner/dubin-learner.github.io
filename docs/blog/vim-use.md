...
### 模糊搜索插件fzf
fzf是模糊搜索工具，在[官方Github主页](https://github.com/junegunn/fzf)里可以找到不同系统编译好的可执行文件。
...
```
离线安装的话则需要先安装fzf工具，到上面提到的官方Github主页中下载源码：
```bash
git clone https://github.com/junegunn/fzf.git
mv fzf ~/.fzf
```
在[Releases页面](https://github.com/junegunn/fzf/releases)中下载系统对应的可执行文件，解压后放到bin路径下：
```bash
tar xvf fzf-*.tar.gz
mkdir ~/.fzf/bin
mv ./fzf-* ~/.fzf/bin/
...
这里读的是bin路径下的`preview.sh`和`tagpreview.sh`两个文件，生成的preview命令；这两个文件的内容也差不多。

因此，可以直接修改这两个文件中调用cat的位置，如下：
```bash
